...
function ProfileProject() {
  ...
  function getRandomColors() {
    const colors = ['rgb(0, 174, 82)', 'RGB(248, 222, 0)', 'RGB(37, 126, 188)', ' RGB(248, 144, 0)'];
    const randomColors = [];
  ...
  return (
    <div className='flex flex-col mt-6 overflow-y-scroll h-[98vh]'>
      <div className='flex justify-around items-center'>
        ...
        <IoMdReturnLeft size={35} onClick={goBack} style={{ cursor: 'pointer' }} />
        </div>
        <h1 className='text-6xl font-[900] font-league-spartan'>Team's Profile</h1>
        <Link to="/final-result/:id" className="text-gray-400" onClick={handleFinalResultClick}>
          See final result
        </Link>
      </div>
      <div>
        <div className='flex items-center justify-around mt-10'>
           ...
           <img src={teamPic} alt="" />
           <div className='pl-10'>
            <h1 className='text-3xl font-bold font-league-spartan'>TEAM NAME</h1>
            <p className='text-gray-400 text-xl'>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique ad architecto unde harum fugit</p>
           </div>
           </div>
           ...
           {skills.map((item, index) => {
                ...
                return (
                    ...
                    <button key={index}  className=' px-3 py-3 rounded-full' style={{ backgroundColor: randomColor[index] }}>
                        {item }
                    </button>
                );
            })}
      ...
      <div className='flex flex-col mt-10 w-[62%] items-center'>
        ...
        {Links.map((items , index) =>(
            <li className='text-blue-500 justify-start underline'><a href="">{items}</a></li>
        ))}
        </ul>
      </div>
      <div className='flex flex-col mt-10 w-[62%] items-center'>
        <h1 className='text-3xl font-bold'>Check Out those Links :</h1>
       <div className='flex mt-4 ml-40 w-[55%]'>
        {criterias.map((cri , index) =>(
          <div className='px-4 mx-2 w-full py-2 bg-opacity-62 bg-gray-200  rounded-2xl'  >
            <h1 className='text-[20px] font-bold'><a href="">{cri}</a></h1>
        ...
        <button className='px-4 mx-2 py-2 bg-opacity-62 bg-gray-200 items-center flex flex-col   rounded-2xl'>
           < IoAddCircleOutline size={20}/>
            <span className='text-gray-500 text-[10px] '>Add Criteria</span>
          </button>
      </div>
      </div>
    ...
    <input  type="text"
                ...
                className='bg-opacity-62 mt-4 bg-gray-200 w-[61%] rounded-2xl h-[120px] outline-none resize-none p-4'
                value={feedback}
